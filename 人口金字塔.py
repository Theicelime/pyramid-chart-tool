...
# --- ⬇️ 修复：移除了 Columns 布局，使其垂直堆叠 ⬇️ ---
if export_unit == "毫米 (mm)":
    export_width_mm = st.sidebar.number_input("导出宽度 (mm)", 10, 500, 150, 10, key="export_w_mm")
    export_height_mm = st.sidebar.number_input("导出高度 (mm)", 10, 500, 100, 10, key="export_h_mm")
    # 设置 px 的默认值 (以防万一)
    export_width_px, export_height_px = 1200, 700 
else: # "像素 (px)"
    export_width_px = st.sidebar.number_input("导出宽度 (px)", 500, 5000, 1200, 100, key="export_w_px")
    export_height_px = st.sidebar.number_input("导出高度 (px)", 300, 5000, 700, 100, key="export_h_px")
    # 设置 mm 的默认值 (以防万一)
...
if uploaded_file is not None:
    try:
        ...
        if all(col in df.columns for col in required_cols):
            ...
            # --- 修复：替换 use_container_width 为 width='stretch' ---
            ...
            # --- 确保 `calc_export_width` 和 `calc_export_height` 在传递前已定义 ---
            ...
            img_png = fig.to_image(format="png", width=calc_export_width, height=calc_export_height, scale=png_scale_factor)
            ...
            with col3:
                ...
                st.caption(f"PNG 最终像素: {int(calc_export_width * png_scale_factor)} x {int(calc_export_height * png_scale_factor)}")
